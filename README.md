...
4. Full model for inference - The easiest way to run the full application is by pulling it from my dockerhub. I created several options in regards to the LLM used because LLM's cause the docker image to grow by more than the size of the LLM:
    -The first two images create containers that are fully packaged with the model included from Hugging Face. All you do is paste the line to a terminal and type "localhost" in a browser. Given the image sizes grow large when dealing with LLM's and Pytorch, the first one ("doglite") is recommended. The first lighter model is prithivMLmods/Llama-SmolTalk-3, while the second uses HuggingFaceTB/SmolLM2-1.7B-Instruct
`docker run -it -p 80:80/tcp chrismontes22/litedog`
`docker run -it -p 80:80/tcp chrismontes22/full_dog_app_fastapi_inference`
    -The following image comes without a model packaged, but you can use it by simply mounting a text generation model of your choice form Hugging Face. Save the necessary LLM files to a folder and mount the folder to the container. Some recommended models are the ones mentioned above:
    Bash
