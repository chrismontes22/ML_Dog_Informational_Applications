...
2. Image Classification App - The best way to see my Image Classification application is to head on over to my Hugging Face Space, [chrismontes/Dog_Breed_Identifier](https://huggingface.co/spaces/chrismontes/Dog_Breed_Identifier). This model has been trained through trnsfer learning on thousands of images, over multiple training and data cleaning cycles. Another way to run my app is to run a Docker Image to create a container with the necessary dependencies and inference script. If interested in using docker, please scroll to the bottom for details.
